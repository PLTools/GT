...
Recursive function ``$\inbr{show_{expr}}$'' (the first evident candidate for generic implementation) converts an expresion into its string representation: \marginnote{Printf format has \%S modifier which inserts quotes around string. Let's use that!}
...
when argument fself is not the last in the list of arguments.}\footnote{ Also trf function name is the same as class name. This can provoke reader to ask questions}\marginnote{I think that arguments `p` and `e` should be removed (looks like and error)}
