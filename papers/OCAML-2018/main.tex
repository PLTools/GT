...
fixpoint combinator). As it can be seen from this example, we needed to implement only ``the interesting'' part of the transformation. All other
...
The \cd{eval} plugin here generates a transformation \cd{eval(lam)}, which is analoguous to \cd{fmap}, but additionally uses some environment, which
...
\begin{itemize}
   ...
   \item \cd{Visitors} in theis current state do not support polymorphic variants.
\end{itemize}
