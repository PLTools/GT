...
type show_t_a_1 = { show_a_trf : unit -> a -> string }
type show_t_b_1 = { show_b_trf : unit -> b -> string }
...
let a =
  {GT.gcata = gcata_a; GT.plugins = object (_) method show = show_a () end}
let show_a = show_a ()
let b =
  {GT.gcata = gcata_b; GT.plugins = object (_) method show = show_b () end}
let show_b = show_b ()
