...
class ['a, 'v] var_eval = object
  ...
  method m_Var s v name = try List.assoc name s with Not_found -> v.GT.x
end
...
class ['a, 'v] lambda_eval = object
  ...
  method m_Abs s v name l1 = 
    let s' = gensym () in
   `Abs (s', v.GT.f ((name,`Var s')::s) l1.GT.x)
  method m_App s v l1 l2 =
...
class ['a] var_expr_eval = object
  ...
  method m_Num  s v i   = v.GT.x
  method m_Add  s v x y = 
    match x.GT.fx s, y.GT.fx s with
    | `Num x, `Num y -> `Num (x+y)
    | x, y -> `Add (x, y) 
  method m_Mult s v x y =
    match x.GT.fx s, y.GT.fx s with
    | `Num x, `Num y -> `Num (x*y)
    | x, y -> `Mult (x, y)
 end
