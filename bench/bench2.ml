...
module Lambda = struct
  ...
  module Id = struct
    ...
    module G = struct
      ...
      let f inh xs =
        ...
        and fself () x = gcata (Lazy.force obj) inh x
        in
        fself inh xs
    end
   end
  ...
  module PP = struct
    ...
    module M = struct
      let work =
        ...
        ; app = (fun st ppf l r -> Format.fprintf ppf "(%a %a)" (apply_strat st) l (apply_strat st) r)
        ; abs = (fun st ppf s b -> Format.fprintf ppf "(\ %s -> %a)" s (apply_strat st) b)
        }
    ...
    module G = struct
      ...
      let f ppf xs =
        let rec obj = lazy (o fself)
        and fself ppf x = gcata (Lazy.force obj) ppf  x
        in
        fself ppf xs
    end
  end
  ...
  module Eval = struct
    ...
    module V = struct
      let o = object(self)
        method c_Var i s = Var s
        method c_Abs ppf x b = Abs (x,b)
