(** Class types with methods that all plugins should have implemented.
    ...
    plugin generate a {i plugin class} called [pluginname_typ_t] and
    a {i transformation function} called [pluginname_t].
